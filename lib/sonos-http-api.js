...
function HttpAPI(discovery, port, presets) {
  ...
  function restoreSonos(callback) {
    ...
    //console.log("count cordinatorState: " + coordinatorState.length);
    console.log(utils.inspect(coordinatorState, false, 7));
    ...
    coordinatorState.forEach(function (coordinator) {
      //console.log(utils.inspect(coordinator, false, 7));
      ...
      asyncSeries.push(function (callback) {
        playerCoordinator.becomeCoordinatorOfStandaloneGroup(function (success) {
          console.log("Isolated: " + playerCoordinator.roomName);
          callback(null, success);
        });
      });
      ...
      coordinator.members.forEach(function (member) {
        ...
        //Now, while looping through each player, let's group it to the biggest group
        var streamUrl = "x-rincon:" + coordinatorBackedUp.uuid;
        ...
        if (player.avTransportUri !== streamUrl && player.uuid !== coordinatorBackedUp.uuid) {
          ...
          console.log(player.roomName + "/" + player.uuid + " - AvTransportUri already is grouped to: " + coordinatorBackedUp.uuid);
          //console.log("coordinatorBackedUp.uuid: " + coordinatorBackedUp.uuid);
        }
      });
    });
