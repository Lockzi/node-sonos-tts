...
TextToSpeech.prototype.generateTTS = function (text, callback) {
  ...
  this.verifyFileExistance(filepath, function (err, filepath) {
    if (err) {
      err = null;
...
//action TTSALL
TextToSpeech.prototype.getTTSURI = function (text, req, callback) {
  ...
  this.generateTTS(text, function (err, TTS) {
    ...
    var TTSURI = "http://10.0.3.215:5006/stream/" + TTS;
...
TextToSpeech.prototype.stream = function (filename, callback) {
  var _this = this;
  console.log("Streaming action!");

  var filepath = this.convertFilenameToFilepath(filename);
  ...
  this.verifyFileExistance(filepath, function (err, filepath, size) {
    ...
    if (err) {
      ...
      filepath = _this.convertTextToFilepath("Error. Message does not exist");

      console.log("Error filepath: " + filepath);

      _this.verifyFileExistance(filepath, function (err, filepath, size) {
        if (err) {
